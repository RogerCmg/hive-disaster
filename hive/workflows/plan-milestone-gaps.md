<purpose>
Create all phases necessary to close gaps identified by `/hive:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/hive:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/hive:audit-milestone` first.
```
...
PHASES=$(node ~/.claude/hive/bin/hive-tools.js phases list)
...
```bash
node ~/.claude/hive/bin/hive-tools.js commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md
```
...
**Also available:**
- `/hive:execute-phase {N}` — if plans already exist
...
`/hive:audit-milestone` — re-audit to verify gaps closed
`/hive:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/hive:plan-phase` next
</success_criteria>
