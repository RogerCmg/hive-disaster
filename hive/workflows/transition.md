...
**Delegate ROADMAP.md and STATE.md updates to hive-tools:**

```bash
TRANSITION=$(node ~/.claude/hive/bin/hive-tools.js phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `hive-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node ~/.claude/hive/bin/hive-tools.js progress bar --raw)
```
...
**Use the transition result from `hive-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/hive:plan-phase [X+1]")
...
**Also available:**
- `/hive:discuss-phase [X+1]` — gather context first
- `/hive:research-phase [X+1]` — investigate unknowns
- Review roadmap
...
Exit skill and invoke SlashCommand("/hive:complete-milestone {version}")
