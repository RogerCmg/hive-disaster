...
```bash
PHASE_INFO=$(node ~/.claude/hive/bin/hive-tools.js find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/hive/bin/hive-tools.js roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/hive/bin/hive-tools.js find-phase "${PHASE}" --raw)
```
