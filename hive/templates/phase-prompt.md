...
> **Note:** Planning methodology is in `agents/hive-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/hive/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/hive:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/hive/references/tdd.md` for TDD plan structure.
...
See `~/.claude/hive/templates/user-setup.md` for full schema and examples
...
See `~/.claude/hive/workflows/verify-phase.md` for verification logic.
