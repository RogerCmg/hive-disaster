...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node ~/.claude/hive/bin/hive-tools.js summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
**Delegate archival to hive-tools:**

```bash
ARCHIVE=$(node ~/.claude/hive/bin/hive-tools.js milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
- [ ] User knows next step (/hive:new-milestone)
